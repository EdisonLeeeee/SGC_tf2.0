...
class Dataset(object):
    def __init__(self, dataset, seed=15):
        self.name = dataset
        self.adj, self.features, self.true_labels = self.load_npz(dataset)

        self.n_nodes = self.adj.shape[0]
        self.n_features = self.features.shape[1]
        self.n_classes = self.true_labels.max() + 1

        self.adj_norm = preprocess_adj(self.adj)
        self.feat, self.precompute_time = sgc_precompute(
            self.features, self.adj_norm)

        feat_dense = self.feat.todense()

        self.idx_train, self.idx_val, self.idx_test = train_val_test_split_tabular(
            self.n_nodes, stratify=self.true_labels, random_state=seed)

        self.train_feat = feat_dense[self.idx_train]
        self.val_feat = feat_dense[self.idx_val]
        self.test_feat = feat_dense[self.idx_test]
        self.y_train = self.true_labels[self.idx_train]
        self.y_val = self.true_labels[self.idx_val]
        self.y_test = self.true_labels[self.idx_test]
